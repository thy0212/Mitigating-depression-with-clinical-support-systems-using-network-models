...
def mfq_without_network(
    ...
    :return:
    """
    mfq = np.zeros(num_episode, dtype=int)
    mfq[0] = current_mfq
    for t in range(1, num_episode):
...
def mfq_with_network(
    ...
    mfq = np.zeros(num_episodes,dtype=int)
    mfq[0] = initial_mfq
    for t in range(1, num_episodes):
        if mfq[t - 1] >= 28:
            mfq[t] = round(
                mfq[0] + 0.32 * mfq[t - 1] - 0.2 * family_support - 0.34 * friend_support + np.random.normal(0, 1)
            )
        else:
            mfq[t] = round(mfq[0] + 0.32 * mfq[t - 1] + np.random.normal(0, 1))
        if mfq[t] > 66:
...
def generate_mfq(
    ...
    return MFQScore(
        ...
        mfq_with_network=mfq_with_network(initial_mfq, num_episodes, family_support, friend_support),
    )
